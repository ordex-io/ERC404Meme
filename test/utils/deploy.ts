...
export async function deployUniswapPool() {
  ...
  for (let i = 0; i < 10; i++) {
    try {
      ...
      const erc404Address =
        await PET404ContractsData.diamondContract.getAddress(); // token1
      // (0.05, 0.3, 1, 0.01)
      const fee = 0.3 * 10000;
      ...
      const erc404Decimals =
        await PET404ContractsData.diamondContract.decimals();
      const amountErc404 =
      ...
      // NOTE: There is an issue here, we get 'STF' error which is an Uniswap error for `safeTransferFrom` library.
      // For some reason is failing sometimes. It is not an issue with the deploy function, missing approves or a wrong
      // value because if we re-order some test cases, the function pass. I guess is some issue with the HH node and
      // an error with multiple instances deployed one after other. Until that, we try to force the deployment
      // If the whole force fail, then something else is happening. We should be refactoring this an try to really figure
      // out what is happening.
      //
      // This errors array is to log if at the end everything fail
      errors.push(error!.toString());
    }
  }
  console.log(JSON.stringify(errors, null, 2));
  throw new Error(
    "An error happened. RUn again the tests, this is just a workaround for the uniswap issue that sometimes happen. Check the test logs"
  );
}
