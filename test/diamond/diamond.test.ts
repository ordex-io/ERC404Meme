...
import { deployFullPET404DiamondVrf } from "../utils/deploy";
...
describe("Diamond - NFT - CAT404", () => {
  describe("Facets with Auto Non VRF", () => {
    it("should use the correct owner address for different facets", async () => {
      ...
      const { diamondContract, ownerSigner, facetsArgs } =
      ...
      expect(
        diamondContract
          .connect(signer0)
          ["mintERC20(address,uint256)"](transferExemptAddress, amountToMint)
      ).to.be.revertedWithCustomError(diamondContract, "Ownable__NotOwner");
    ...
    it("should mint NFT only if get enough ERC20 tokens", async () => {
      ...
      await diamondContract
        .connect(ownerSigner)
        ["mintERC20(address,uint256)"](signer1.address, erc20Amount);
  ...
  describe("Facets with Auto VRF", () => {
    it("should use the correct owner address for different facets", async () => {
      const [signer0, signer1] = await ethers.getSigners();
      const { diamondContract, ownerSigner, facetsArgs } =
        await deployFullPET404DiamondVrf();
    ...
    it("should mint NFT only if get enough ERC20 tokens", async () => {
      ...
      const result1 = await checkUpKeepCall(diamondContract, ethers.provider);

      expect(result1.upkeepNeeded).to.be.false;

      await time.increase(facetsArgs.automation.minWait_);

      const result2 = await checkUpKeepCall(diamondContract, ethers.provider);
